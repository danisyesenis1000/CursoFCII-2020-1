//A continuación se realiza el punto 11
...
int main(){
  ...
  cout<<"Ingrese el conjunto de cadenas que desea ingresar, cada cadena la separa un enter"<<endl;
  ...
  for(i=0;i<Nca;i++){  //Se reemplaza la letra evaluando cada caracter de cada palabra
    ...
    for(int j=0;j<cl;j++){
      for(int k=0;k<cr;k++){
	do{
	  if (palabrai[j]==caracteres[k]){ 
	    palabrai[j]=caracteresn[k];
	  }
	}while(palabrai[j]!=caracteres[k]);
      }
    }
