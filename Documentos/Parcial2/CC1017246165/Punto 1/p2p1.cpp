...
//Se crea una función general que evidencia como hereda

void listas(double R,double FA,double IT,double TT,int NT,double F,double P,double V){
  for(int i=0;i<=NT;i++){
    double ITL=i*IT;
    Espiral espira(R,FA,ITL,TT,NT,F,P,V);
...
int main(){
  ...
  if(elect==1){ //Se usa de acuerdo a los datos dados por el programa
    ...
    espira1.Datosxyz();
    espira1.zpos1();
    listas(r1,fa1,it1,tt1,nt1,f1,p1,v1);
  }
    ...
    espira2.Datosxyz();
    espira2.zpos1();
    listas(r2,fa2,it2,tt2,nt2,f2,p2,v2);
  }
